...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: "mkdir --mode=0700 /root/.ssh"
  config.vm.provision "file", source: "~/.ssh/id_rsa.pub", destination: "/tmp/authorized_keys"
  config.vm.provision "shell", inline: "mv /tmp/authorized_keys /root/.ssh/"
  config.vm.provision "shell", inline: "chmod 600 /root/.ssh/authorized_keys"
  config.vm.provision "shell", inline: "chown root:root /root/.ssh/authorized_keys"
  #   apt-get update
